...
$output=$ARGV[0]."-".$ARGV[1].".second.fst";
		open(W,">$output");
  ...
  $ht=0;$hs1=0;$hs2=0;
   ...
   if($_<=$en{$n} and $_>=$start{$n}){
 		...
 		if($ht==0 |$hs1==0 or  $hs2==0 or $ht_count==0 or $hs_count1==0 or $hs_count2==0){$Fst{$n}="NA";}
 	 	    elsif($ht ne "0"){$Fst{$n}=1-0.5*($hs1/$hs_count1+$hs2/$hs_count2)/(0.5*(($ht/$ht_count)+0.5*($hs1/$hs_count1+$hs2/$hs_count2)));
			}# if($ht ne "0"){$Fst=1-($hs/(0.5*$hs+0.5*$ht))}
